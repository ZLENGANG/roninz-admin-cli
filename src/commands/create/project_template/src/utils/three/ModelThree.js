...
// 创建线
const createLineMesh = (color, linewidth, points) => {
...
// 创建面
const createFaceMesh = (array, color) => {
    const heartShape = new THREE.Shape(array.map((item) => new THREE.Vector2(item.x, item.z)));
...
// 获取模型动画
const getActions = (animations, model, activeAction = "") => {
    const actions = {};
    const mixerArray = [];
    if (animations && animations.length > 0) {
        ...
        if (activeAction) {
            const activeActionPlay = actions[activeAction];
...
export default class ModelThree {
    constructor(scene, camera, renderer, params, db) {
        this.render = (dt) => {
            if (this.modelMap) {
                this.modelMap.forEach((actionsValue) => {
                    if (Object.hasOwn(actionsValue, "mixerArray")) {
                        actionsValue["mixerArray"].forEach((item) => {
                            item.update(dt);
                        });
                    }
                });
         ...
         * @returns
         */
        this.getModel = (name, childrenName = "") => {
            let model = this.modelGroup.getObjectByName(name);
            if (!model)
                model = this.scene.getObjectByName(name);
        ...
        const load = (db) => {
            getData(db, "model", url)
                ...
                if (v) {
                    ...
                    that.dbLoad(Object.assign(Object.assign({}, modelParams), { url: path }), callBack);
                }
                else {
                    const loadXhr = new Xhr();
                    ...
                    xhr.onload = () => {
                        if (xhr.status === 200) {
                            ...
                            that.dbLoad(Object.assign(Object.assign({}, modelParams), { url: path }), callBack, loadXhr);
                        }
                        else {
    ...
    // 加载模型
    dbLoad(modelParams, callBack, load) {
        const { KTX2, url, position: { x = 0, y = 0, z = 0 }, scale, name, rotation, activeAction, DRACO, } = modelParams;
        let loader = null;
        ...
        loader.load(url, (gltf) => {
            ...
            const { actions, mixerArray } = getActions(gltf.animations, model, activeAction);
     ...
     * @returns
     */
    playActiveAction(name, playKey, flag = true) {
    ...
    isInModel(params) {
        ...
        return boundingBox.containsPoint(new THREE.Vector3(point.x, point.y, point.z));
    }
    getBoxHelp(name, childrenName = "") {
        const model = this.getModel(name, childrenName);
        if (!model)
            return;
     ...
     * @returns
     */
    modelMove(name, position, animation, speed, callback, run) {
        const model = this.getModel(name);
        if (!model)
            return;
        if (!animation) {
    ...
    clickModelMove(modelPosition, Position) {
        const startPosition = new THREE.Vector3(modelPosition.x, 0, modelPosition.z); // 起始位置
     ...
     * @returns void
     */
    addShapeGeometry(pathArray, params, callBack) {
        const { name, color = "#FF962C", linewidth = 4, type = "line", visible = true, } = params;
        if (this.electronRail.get(name))
            return;
        const array = [];
    ...
    // 删除多边形围栏
    deletGeometry(name) {
        const model = this.electronRailGroup.getObjectByName(name);
        if (model) {
            this.modelGroup.remove(model);
    ...
    // 多边形围栏显示隐藏
    showGeometry(name, visible = true) {
        const model = this.electronRailGroup.getObjectByName(name);
        if (!model)
            return;
        model.visible = visible;
