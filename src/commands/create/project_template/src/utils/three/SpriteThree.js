...
export default class SpriteThree {
    ...
    //绘制加载动画
    createLoading(value, schedule, color = "#00F3B8") {
        ...
        ctx.arcTo(startWidth + x - left, ctxHeight, startWidth + x - left, ctxHeight + boxHeight, 10);
        ctx.arcTo(startWidth + x - left, ctxHeight + boxHeight, startWidth + left, ctxHeight + boxHeight, 10);
        ctx.arcTo(startWidth + left, ctxHeight + boxHeight, startWidth + left, ctxHeight, 10);
        ctx.arcTo(startWidth + left, ctxHeight, startWidth + x - left, ctxHeight, 10);
        ctx.stroke();
        const border = 8;
        ...
        for (let i = 0; i < index; i++) {
            ctx.fillRect(progressLeft + startWidth, ctxHeight, loadingWidth, boxHeight - border * 2);
            ...
            if (addWidth > border && i === num - 1) {
                ctx.fillRect(progressLeft + startWidth, ctxHeight, addWidth, boxHeight - border * 2);
            }
        }
        ctx.font = font + "px Calibri";
        ctx.strokeStyle = color;
        ...
        ctx.fillText(text1, startWidth + x - textSchedule.width - (left - textSchedule.width) / 2, ctxHeight + boxHeight / 2);
        ctx.restore();
        return canvas;
    }
    // 添加精灵图标签
    addLabel(labelParams) {
        const { name, element, type = "CSS2DObject", position = { x: 0, y: 0, z: 0 }, scale = { x: 1, y: 1, z: 1 }, } = labelParams;
    ...
    // 添加线
    addLine(coordArray, lineName = "") {
        const points = coordArray.map((p) => new THREE.Vector3(p.x, p.y, p.z));
        const curve = new THREE.CatmullRomCurve3(points, false);
        const line = new THREE.Line(new THREE.BufferGeometry().setFromPoints(curve.getPoints(points.length * 10)), new THREE.LineBasicMaterial({
            color: 0x0000ff,
        }));
        line.name = lineName;
        this.scene.add(line);
