...
const asyncRouter = (routerList) => {
    if (!routerList || routerList.length === 0)
        return [];
    // 路由过滤
    const filterRouterList = routerList.filter((item) => pages.find((path) => path === item.component || item.component.indexOf("https") !== -1));
    return filterRouterList.map((r) => ({
        path: r.path,
        name: r.name,
        component: r.component.indexOf("https") === -1
...
//动态添加路由
const addRouter = async (userInfoStore) => {
